...
function mapConceptSummary(raw: Record<string, unknown> | undefined): ConceptSummary {
  ...
  return {
    nameSuggestion: pick(
      raw.nameSuggestion as string,
      raw.name_suggestion as string,
    ),
    oneLineSummary:
      pick(raw.oneLineSummary as string, raw.one_line_summary as string) ||
      'Your venture',
...
function mapMarketScan(raw: Record<string, unknown> | undefined): MarketScan {
  ...
  return {
    ...
    opportunityGaps: asArray<string>(
      raw.opportunityGaps ?? raw.opportunity_gaps,
    ),
...
function mapPitchDeck(raw: unknown): PitchDeckSlide[] {
  return asArray<Record<string, unknown>>(raw).map((s, i) => ({
    slideNumber: (s.slideNumber as number) ?? (s.slide_number as number) ?? i + 1,
    title: (s.title as string) || `Slide ${i + 1}`,
    mainMessage: pick(s.mainMessage as string, s.main_message as string) || '',
    bullets: asArray<string>(s.bullets),
...
function mapInvestorQA(raw: unknown): InvestorQA[] {
  return asArray<Record<string, unknown>>(raw).map((q) => ({
    ...
    answerFramework: pick(q.answerFramework as string, q.answer_framework as string) || '',
...
function mapMarketing(raw: Record<string, unknown> | undefined): MarketingStarterPack {
  ...
  return {
    ...
    coldEmail: raw.coldEmail
      ? (raw.coldEmail as MarketingStarterPack['coldEmail'])
      : raw.cold_email
        ? (raw.cold_email as MarketingStarterPack['coldEmail'])
        : undefined,
...
export function mapSessionToPitchResult(session: BackendSession): PitchGenerationResult {
  ...
  const jobMarketing = asRecord(pitchOut?.marketingPack ?? pitchOut?.marketing_pack)
  ...
  const marketingStarterPack = mapMarketing(
    jobMarketing ??
      asRecord(pitchOut?.marketingStarterPack) ??
      asRecord(pitchOut?.marketing_starter_pack),
  )
